...
-- 导出 wison_oss 的数据库结构
DROP DATABASE IF EXISTS `wison_oss`;
CREATE DATABASE IF NOT EXISTS `wison_oss` /*!40100 DEFAULT CHARACTER SET utf8mb4 */;
USE `wison_oss`;

-- 导出  表 wison_oss.upload_file_log 结构
DROP TABLE IF EXISTS `upload_file_log`;
CREATE TABLE IF NOT EXISTS `upload_file_log`
(
    `id`                  char(20)     NOT NULL COMMENT '分布式唯一ID',
    `source_service`      varchar(255) NOT NULL COMMENT '来源系统名称',
    `source_key`          varchar(50)  DEFAULT NULL COMMENT '来源系统ID',
    `login_code`          varchar(50)  NOT NULL COMMENT '创建人登录名',
    `zone_code`           varchar(5)   NOT NULL COMMENT '板块编码',
